...
public class LogsCenter {
    ...
    static {
        logger = Logger.getLogger(LogsCenter.class.getName()); // Must precede setBaseLogger to initialize logger first.
        LogsCenter.setBaseLogger(); // If logger not initialized, NullPointerException causes both CLI and GUI to fail.
    }
    ...
    public static void init(Config config) {
        ...
        LogsCenter.logger.info("Log level will be set as: " + currentLogLevel);
        // set the level of the baseLogger which will be inherited by other loggers
        LogsCenter.baseLogger.setLevel(currentLogLevel);
    }
    ...
    public static Logger getLogger(String name) {
        ...
        LogsCenter.removeHandlers(logger);
        LogsCenter.logger.setUseParentHandlers(true);
        return logger;
    }
     ...
     */
    public static <T> Logger getLogger(Class<T> classToHaveLogger) {
        requireNonNull(classToHaveLogger);
        return LogsCenter.getLogger(classToHaveLogger.getSimpleName());
    }
    ...
    private static void setBaseLogger() {
        LogsCenter.baseLogger = Logger.getLogger("ab3");
        LogsCenter.baseLogger.setUseParentHandlers(false);
        ...
        LogsCenter.baseLogger.addHandler(consoleHandler);

        // add a FileHandler to log to a file
        try {
            FileHandler fileHandler = new FileHandler(LogsCenter.LOG_FILE,
                    LogsCenter.MAX_FILE_SIZE_IN_BYTES,
                    LogsCenter.MAX_FILE_COUNT,
                    true);
            fileHandler.setFormatter(new SimpleFormatter());
            fileHandler.setLevel(Level.ALL);
            LogsCenter.baseLogger.addHandler(fileHandler);
        } catch (IOException e) {
            LogsCenter.logger.warning("Error adding file handler for logger.");
        }
    }
}
